  ...
  <v-app>
    <v-overlay v-model="store.overlay" class="justify-center align-center">
      <v-card class="mx-auto" max-width="800px" light>
        <v-img class="align-end text-white" height="150" src="https://ssl.gstatic.com/atari/images/impression-header.png"
          cover>
      ...
      Visualizing Conflict Resolution Tasks
        ...
        <v-card-text>
          <div>
            <p
              style="padding: 10px; border-radius: 10px; border: 1px dashed orange; font-family:monospace; color: orange">
              Di Salvatore, Jessica, Lundgren, Magnus, Oksamytna, Kseniya, & Smidt, Hannah M. (2022). Introducing the
              Peacekeeping Mandates (PEMA) Dataset. <a href="https://doi.org/10.1177/00220027211068897">Journal of
                Conflict Resolution</a> 66 (4-5): 924-951.</p>
          </div>
          <p>Since the end of the Cold War, UN peacekeeping operations have been given an increasing num-ber of ambitious
            tasks, ranging from promoting human rights to organizing elections. </p>
          <p>Drawing on UN Security Council resolutions that establish, extend, or revise mandates of 48 UN peacekeeping
            operations in 1991–2016, the Peacekeeping Mandates (PEMA) Dataset records 41 distinct tasks, three modalities
            of engagement (monitoring, assisting, and securing), and whether the task is requested or encouraged.</p>
          <p>The Codebook and the data (with future updates) can be found on the <a
              href="https://dataverse.harvard.edu/dataset.xhtml?persistentId=doi:10.7910/DVN/OT14Z9">Harvard
              Dataverse</a>.</p>
          <v-card-actions>
          ...
          <p><small style="font-family: monospace">We gratefully acknowledge funding from Folke Bernadotte Academy (dnr.
              19-00288) and Higher Education Innovation Fund by UK Research and Education awarded through City, University
              of London (project no. 717113).</small></p>

          <v-btn size="x-small" variant="plain" color="lighten"> <a href="https://tobiaskauer.org">Visualization & Design
              byTobias Kauer</a></v-btn>
        </v-card-text>
      </v-card>
    ...
    <v-app-bar app clipped-left color="primary" dark>
      ...
      <v-btn-toggle style="padding-right: 20px" v-model="store.view" color="primary" mandatory size="x-small">
    ...
    <v-navigation-drawer v-model="store.showFilters" width="400">

      <ResolutionFilters v-if="resolutions.length > 0" :resolutions="resolutions" :filters="filters" :tasks="tasks"
        @change-filters="updateFilters" @hide-filters="store.showFilters = !store.showFilters" />
    </v-navigation-drawer>
    <v-main>
      <VisualFingerprint v-if="store.view == 'vis'" :resolutions="filteredResolutions" :tasks="tasks"
        :modalities="modalities" :activeTasks="filters.tasks" />
      <ResolutionTable v-if="store.view == 'table'" :resolutions="filteredResolutions" :filters="filters"
        :tasks="tasks" />
    </v-main>
  </v-app>
</template>
    ...
    tasks.forEach(task => {
      ...
      if (task.modalities) modalities.forEach(modality => {
        if (resolution[task.key + modality.name]) {
          ...
          resolution[task.key] += resolution[task.key + modality.name]
        }
      })
    })
